      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-outline-variant pt-3 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-outline mb-1 block">Date</label>
              <input
                type="date"
                value={entry.date}
            ...
            <div>
              <label className="text-xs font-medium text-outline mb-1 block">Type</label>
              <select
                value={entry.type}
          ...
          <div>
            <label className="text-xs font-medium text-outline mb-1 block">Parties Involved</label>
            <input
              type="text"
              value={entry.parties}
          ...
          <div>
            <label className="text-xs font-medium text-outline mb-1 block">Outcome / Action Required</label>
            <input
              type="text"
              value={entry.outcome}
          ...
          <div>
            <label className="text-xs font-medium text-outline mb-1 block">Notes / Reference</label>
            <input
              type="text"
              value={entry.notes}
          ...
          {entry.addedBy && (
            <p className="text-xs text-on-surface-variant">Added by {entry.addedBy.name} · {fmtTs(entry.addedBy.at)}</p>
          )}
          {(entry.teamNotes ?? []).map((n) => (
            <div key={n.id} className="mt-1 pl-3 border-l-2 border-outline-variant text-xs text-outline">
...
function TodoItem({ todo, onUpdate, onRemove }) {
  ...
  return (
    <div className={`rounded-3xl border overflow-hidden transition-colors ${todo.done ? 'border-primary/20 bg-primary/5' : 'shadow-md border border-outline-variant/10 bg-surface-container-lowest'}`}>
      <div className="flex items-center gap-3 px-4 py-3">
        <input
          type="checkbox"
          checked={!!todo.done}
        ...
        <div className="flex-1 min-w-0">
          <input
            ...
            className={`w-full text-sm bg-transparent border-b border-transparent hover:border-outline-variant focus:border-primary focus:outline-none py-0.5 ${
              todo.done ? 'line-through text-outline' : 'text-on-surface font-medium'
            }`}
          />
          <div className="flex items-center gap-2 mt-0.5 flex-wrap text-xs">
            ...
            {todo.dueDate && (
              <span className={overdue ? 'text-red-500 font-medium' : 'text-outline'}>
                {overdue ? '⚠ ' : ''}Due {formatDate(todo.dueDate)}
              </span>
            )}
            {todo.completedBy && (
              <span className="text-on-surface-variant ml-2">Completed by {todo.completedBy.name}</span>
            )}
            {todo.createdBy && !todo.done && (
              <span className="text-on-surface-variant ml-2">Added by {todo.createdBy.name}</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-1 shrink-0">
          <button
            ...
            className={`p-1 transition-colors ${todo.critical ? 'text-red-500' : 'text-outline hover:text-red-400'}`}
          >
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-outline-variant pt-3 grid grid-cols-2 gap-3">
          <div>
            <label className="text-xs font-medium text-outline mb-1 block">Due Date</label>
            <input
              type="date"
              value={todo.dueDate}
...
export default function CommunicationLog({ project, updateProject }) {
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {/* Tab switcher */}
      <div className="flex bg-outline-variant/50 p-1 rounded-xl mb-5 w-fit">
        <button
          onClick={() => setView('log')}
          className={`text-sm px-4 py-1.5 rounded-xl transition-colors ${view === 'log' ? 'bg-surface-container-lowest text-on-surface shadow-sm font-medium' : 'text-outline hover:text-on-surface'}`}
        >
          💬 Log
          {entries.length > 0 && <span className="ml-1.5 text-xs text-outline">({entries.length})</span>}
        </button>
        <button
          onClick={() => setView('todos')}
          className={`text-sm px-4 py-1.5 rounded-xl transition-colors ${view === 'todos' ? 'bg-surface-container-lowest text-on-surface shadow-sm font-medium' : 'text-outline hover:text-on-surface'}`}
        >
          ✅ To-Dos
          {openTodos.length > 0 && (
            <span className={`ml-1.5 text-xs font-semibold ${overdueTodos.length > 0 ? 'text-red-500' : 'text-primary'}`}>
      ...
      {view === 'log' && (
        <>
              ...
              {presentFilters.map((f) => (
                <button
                  ...
                  className={`text-xs px-3 py-1 rounded-full border transition-colors ${
                    filter === f ? 'bg-primary text-on-primary border-primary' : 'border-outline-variant text-on-surface-variant hover:border-primary hover:text-primary'
                  }`}
                >
                  {f}
          ...
          {entries.length === 0 ? (
            ...
            <div className="text-center py-12 text-outline text-sm">No {filter.toLowerCase()} entries.</div>
          ) : (
        ...
        <>
          {todos.length > 0 && (
            <div className="grid grid-cols-3 gap-3 mb-5">
              <div className={`rounded-3xl border p-6 text-center ${openTodos.length > 0 ? 'bg-amber-50 border-amber-200' : 'shadow-md border border-outline-variant/10 bg-surface-container-lowest'}`}>
                <div className="text-xs text-outline mb-1">Open</div>
                <div className={`text-xl font-bold ${openTodos.length > 0 ? 'text-amber-700' : 'text-on-surface'}`}>{openTodos.length}</div>
              </div>
              <div className={`rounded-3xl border p-6 text-center ${overdueTodos.length > 0 ? 'bg-red-50 border-red-200' : 'shadow-md border border-outline-variant/10 bg-surface-container-lowest'}`}>
                <div className="text-xs text-outline mb-1">Overdue</div>
                <div className={`text-xl font-bold ${overdueTodos.length > 0 ? 'text-red-600' : 'text-on-surface'}`}>{overdueTodos.length}</div>
              </div>
              <div className="bg-green-50 border border-green-200 rounded-3xl p-6 text-center">
                <div className="text-xs text-outline mb-1">Done</div>
          ...
          {todos.length === 0 ? (
            ...
            <div className="space-y-2">
              ...
              {doneTodos.length > 0 && openTodos.length > 0 && (
                <div className="pt-2 pb-1 text-xs text-outline pl-1">Completed</div>
              )}
              {doneTodos.map((todo) => (
                <TodoItem
                  key={todo.id}
