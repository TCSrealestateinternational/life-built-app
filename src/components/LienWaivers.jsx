      ...
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap text-xs text-outline">
            ...
            <span className="text-outline">{waiver.type}</span>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {waiver.status !== 'pending' && (
            <button
              ...
              className="text-outline hover:text-on-surface transition-colors p-1 text-xs"
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-outline-variant pt-3 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-outline mb-1 block">Waiver Type</label>
              <select
                value={waiver.type}
              ...
              {waiver.type && (
                <p className="text-xs text-outline mt-1 italic">{TYPE_DESCRIPTIONS[waiver.type]}</p>
              )}
            </div>
            <div>
              <label className="text-xs font-medium text-outline mb-1 block">Amount Covered ($)</label>
              <input
                type="number"
                value={waiver.amount}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-outline mb-1 block">Through Date</label>
              <input
                type="date"
            ...
            <div>
              <label className="text-xs font-medium text-outline mb-1 block">Date Received</label>
              <input
                type="date"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-outline mb-1 block">Linked Draw / Milestone</label>
              {milestones.length > 0 ? (
                <select
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {waivers.length > 0 && (
        <>
          <div className="grid grid-cols-3 gap-3 mb-5">
            <div className={`rounded-3xl border p-6 text-center ${pending > 0 ? 'bg-amber-50 border-amber-200' : 'bg-surface-container-lowest shadow-md border-outline-variant/10'}`}>
              <div className="text-xs text-outline mb-1">Pending</div>
              <div className={`text-xl font-bold ${pending > 0 ? 'text-amber-700' : 'text-on-surface'}`}>{pending}</div>
            </div>
            <div className="bg-blue-50 border border-blue-200 rounded-3xl p-6 text-center">
              <div className="text-xs text-outline mb-1">Received</div>
            ...
            <div className="bg-green-50 border border-green-200 rounded-3xl p-6 text-center">
              <div className="text-xs text-outline mb-1">Signed</div>
      ...
      {waivers.length === 0 ? (
        ...
        <div className="text-center py-12 text-outline text-sm">No {filter.toLowerCase()} waivers.</div>
      ) : (
