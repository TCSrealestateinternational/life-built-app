...
const PRIORITY_STYLES = {
  ...
  Low:    'bg-linen text-mist border-linen',
};
...
function PunchItem({ item, onUpdate, onRemove }) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden transition-colors ${item.done ? 'border-forest/20 bg-forest/5' : 'bg-white border-linen'}`}>
      <div className="flex items-center gap-3 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <input
            ...
            className={`w-full text-sm bg-transparent border-b border-transparent hover:border-linen focus:border-forest focus:outline-none py-0.5 ${item.done ? 'line-through text-mist' : 'text-ink font-medium'}`}
          />
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-linen pt-3 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-mist mb-1 block">Room / Area</label>
              <input
                type="text"
                value={item.room}
            ...
            <div>
              <label className="text-xs font-medium text-mist mb-1 block">Trade</label>
              <select
                value={item.trade}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs font-medium text-mist mb-1 block">Priority</label>
                ...
                {PRIORITIES.map((p) => (
                  <button
                    ...
                    className={`text-xs px-3 py-1 rounded-full border transition-colors ${
                      item.priority === p ? PRIORITY_STYLES[p] + ' font-semibold' : 'border-linen text-mist hover:border-forest hover:text-forest'
                    }`}
                  >
                    {p}
                  </button>
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      ...
      {items.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          <div className="bg-white border border-linen rounded-xl p-4 text-center">
            <div className="text-xs text-mist mb-1">Total Items</div>
            <div className="text-xl font-bold text-ink">{items.length}</div>
          </div>
          <div className={`rounded-xl border p-4 text-center ${openCount > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-linen'}`}>
            <div className="text-xs text-mist mb-1">Open</div>
            <div className={`text-xl font-bold ${openCount > 0 ? 'text-amber-700' : 'text-ink'}`}>{openCount}</div>
          </div>
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
            <div className="text-xs text-mist mb-1">Complete</div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          ...
          {presentTrades.length > 2 && presentTrades.map((t) => t !== 'All' && (
            <button
              ...
              className={`text-xs px-3 py-1 rounded-full border transition-colors ${
                tradeFilter === t ? 'bg-deep text-white border-deep' : 'border-linen text-mist hover:border-forest hover:text-forest'
              }`}
            >
              {t}
            </button>
      ...
      {items.length === 0 ? (
        ...
        <div className="text-center py-12 text-mist text-sm">No {filter.toLowerCase()} items{tradeFilter !== 'All' ? ` for ${tradeFilter}` : ''}.</div>
      ) : (
